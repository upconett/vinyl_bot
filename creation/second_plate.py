...
def circle_image(photo_path, user_id):
    ...
    # if not os.path.exists(f'creation/img/{user_id}'):
    #     os.mkdir(f'creation/img/{user_id}')
...
def make_video(audio_path, user_id, start_time, noise: bool):
    ...
    if noise:
        ...
        return (f'creation/video/{user_id}/output_video.mp4', f'video/{user_id}/output_video_1m_round.mp4')
    command = [
        'ffmpeg',
        '-y',
        '-stream_loop', '-1',
    ...
    subprocess.run(command)  # Тут делаем видео для кружка
    return (f'creation/video/{user_id}_output_video.mp4', f'video/{user_id}_output_video_1m_round.mp4')
